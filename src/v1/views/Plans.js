...
function Plans() {
  ...
  const TfoData = [
    {
      ...
      BeneFits: [
        ...
        {
          BFname: "Number of Places",
          BFvalue: "1,000+",
          BFlinkText: "View all Places",
          BFlink: "home",
        },
  ...
  const [TouristactiveLinkId, setTouristActiveLinkId] = useState(
    TouristData[0].id
  );
...
useEffect(() => {
  ...
  let plans = state.data?.data;
    ...
    if(state.isLoading){
      ...
      plans && Object.entries(plans?.entertainer).map(([key, val]) => {
                        ...
                        <div className="baobox">
                          ...
                          <div className="bao">
                            {TfoData[0]?.BeneFits.map((item, index) => {
                              return <div key={index}>{item.BFname}</div>;
                            })}
                          </div>
                        </div>

                        {plans?.entertainer.slice().reverse().map((product, index) => (
                          <ComparePlansCard
                            key={index}
                              product={
                                product.product +
                                ` ${Tfoactivebox === product.product_id ? "active" : ""}`
                              }
                            ...
                            saveprice_percent={product.saveprice_percent}
                            billed_annually={product.billed_annually}
                            billed_annually_text={product.billed_annually_text}
                            subscribText={product.subscribText}
                            subscribClass={product.subscribClass}
                            img={product.thumb}
                            LearnMoreAbout_text={product.LearnMoreAbout_text}
                            LearnMoreAbout_link={product.LearnMoreAbout_link}
                            BeneFitsData={product.product_details_json}
                            isSubscribed={plans.user_subscribed_products}
                          />
                        ))}
                      </div>
                    </Tab>
                    <Tab eventKey="tourist" title="Tourist">
                      {TouristData.length > 1 ? (
                        <div className="mobtab">
                      ...
                      <div className="planspg_warp">
                        ...
                        {plans?.tourist.map((product, index) => (
                          <ComparePlansCard
                          key={index}
                            product={
                              product.product +
